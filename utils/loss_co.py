...
class QFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        if self.reduction == 'mean':
            ...
            return loss
def compute_loss_imgs(p,targets,model):#
    p_for_imgs = []
    targets_for_imgs = []
    batch = p[0].shape[0]
    ...
    for _id in range(batch):
        ...
        loss_temp, loss_item_temp = compute_loss(p_for_imgs[_id], targets_for_imgs[_id], model)
...
def compute_loss_coteaching(p,p2 ,targets, model1,model2,remember_rate=0.9):  # predictions, targets, model
    # print('p  ,', p[1].shape)
    # print('targets  :',targets)
    # print('targets  :',targets.shape)
    loss1,loss1_items = compute_loss_imgs(p,targets,model1)
    ...
    loss2,loss2_items = compute_loss_imgs(p2,targets,model2)
    ...
    # print('loss1',torch.sum(loss1)/num_remember)
...
def compute_loss_coteaching2(p,p2 ,targets, model1,model2,remember_rate=0.9):  # predictions, targets, model
    # print('p  ,', p[1].shape)
    # print('targets  :',targets)
    # print('targets  :',targets.shape)
    loss1,loss1_items = compute_loss_imgs(p,targets,model1)
    ...
    loss1_items_update = [loss1_items[i] for i in id2_update]
    ###reweight
    for i in range(num_remember,num):
        loss1_update.append(loss1[sort_id2[i]]*0.5)
    for i in range(num_remember,num):
        loss1_items_update.append(loss1_items[sort_id2[i]]*0.5)
    loss1_items_update_batch = 0
    for _id,loss_items in enumerate(loss1_items_update):
        loss1_items_update_batch += loss_items
    # print('loss1 ',loss1_sorted)
    # print('loss1 items',loss1_items)
    # print('loss1 update',loss1_update)
    # print('loss1_items_update_batch',loss1_items_update_batch)
    loss1_items_update_batch /= num
    loss1_update_batch=0
    for _id,loss1 in enumerate(loss1_update):
        loss1_update_batch += loss1
    loss1_update_batch /= num
    ...
    ###reweight
    for i in range(num_remember,num):
        loss2_update.append(loss2[sort_id1[i]]*0.5)
    for i in range(num_remember,num):
        loss2_items_update.append(loss2_items[sort_id1[i]]*0.5)
    loss2_items_update_batch = 0
    for _id, loss_items in enumerate(loss2_items_update):
        loss2_items_update_batch += loss_items
    loss2_items_update_batch /= num
    loss2_update_batch = 0
    for _id, loss2 in enumerate(loss2_update):
        loss2_update_batch += loss2
    loss2_update_batch /= num
